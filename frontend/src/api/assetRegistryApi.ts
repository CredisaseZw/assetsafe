...
export const assetRegistryApi = {
  ...
  getRecords: async (params?: { asset_type?: AssetType; search?: string }): Promise<AssetRecord[]> => {
    const { data } = await axiosInstance.get<any>('/asset-management/', { params })
    ...
    return records.map((record: any) => ({
      ...
      asset_description: record.description ?? `${record.asset_make ?? ''} ${record.asset_model ?? ''}`.trim(),
