...
export function CollateralForm({ initial, onSuccess, onCancel, isEdit, recordId }: CollateralFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit((d) => submit(d))} className="bg-white">
      ...
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4">
        <div>
          <label className="text-xs font-medium text-slate-600">Financier Type</label>
          <select
      ...
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4">
        <div>
          <label className="text-xs font-medium text-slate-600">Debtor Type</label>
          <select
      ...
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4">
        ...
        <Input label="Year" type="number" {...register('asset_year')} error={errors.asset_year?.message} required />
        <div>
      ...
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-4">
        <div>
          <label className="text-xs font-medium text-slate-600">Currency<span className="text-red-500 ml-0.5">*</span></label>
          <select
            {...register('currency')}
            className="mt-1 h-8 w-full rounded border border-slate-300 bg-white px-2 text-sm focus:outline-none focus:border-[#0f7d8e]"
          >
        ...
        <Input label="Total Debt" type="number" step="0.01" {...register('loan_amount')} error={errors.loan_amount?.message} required />
        <Input label="Instalment Amount" type="number" step="0.01" {...register('instalment_amount')} />
        <Input label="Instalment Date (dd)" type="number" min="1" max="31" {...register('instalment_date')} />
        ...
        <Input label="Agreement Start Date" type="date" {...register('start_date')} error={errors.start_date?.message} required />
        <Input label="Agreement End Date" type="date" {...register('end_date')} error={errors.end_date?.message} required />
      </div>
