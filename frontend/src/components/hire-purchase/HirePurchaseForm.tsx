...
const schema = z.object({
  ...
  asset_condition: z.enum(['new', 'second_hand', 'reconditioned', 'non_functioning']),
...
export function HirePurchaseForm({
  ...
  const handleSaveAndAdd = handleSubmit((data) => {
    submit(data, { onSuccess: () => { reset(); onSaveAndAdd?.() } })
  })
  ...
  return (
    <form onSubmit={handleSubmit((d) => submit(d))} className="bg-white">
      ...
      <div className="flex gap-2 px-4 pt-4 pb-1">
        <Button type="button" size="sm" variant="primary" leftIcon={<UserPlus className="h-3 w-3" />}>
          + Add Individual
        </Button>
        <Button type="button" size="sm" variant="secondary" leftIcon={<Building className="h-3 w-3" />}>
          + Add Company
        </Button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3">
        <div>
          ...
          {errors.financier_id && <p className="text-xs text-red-500">{errors.financier_id.message}</p>}
        </div>
        <Input
          label="Data Date"
          type="date"
      ...
      <div className="p-4 space-y-3">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div>
            <label className="text-xs font-medium text-slate-600">Purchaser Type</label>
            <select
        ...
        <div className="rounded border border-slate-200 bg-slate-50 p-3">
          <p className="mb-2 text-xs font-semibold text-slate-600 uppercase">
            Search: {watch('purchaser_type') === 'individual' ? 'Individual' : 'Company'}
          </p>
          ...
          <p className="mt-2 text-xs text-slate-400 italic">No data available</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3">
        ...
        <Input label="Purchase Amount" type="number" step="0.01" {...register('purchase_amount')} required />
        <Input label="Instalment Amount" type="number" step="0.01" {...register('instalment_amount')} required />
        <Input label="Instalment Date (dd)" type="number" min="1" max="31" {...register('instalment_date')} required />
        <Input label="Total Paid to Date" type="number" step="0.01" {...register('total_paid_to_date')} />
        <Input label="Balance" type="number" step="0.01" {...register('balance')} />
        <Input label="Agreement Start Date" type="date" {...register('start_date')} required error={errors.start_date?.message} />
        <Input label="Agreement End Date" type="date" {...register('end_date')} required error={errors.end_date?.message} />
      </div>

      {/* ── Footer ── */}
      <div className="flex items-center justify-between border-t border-slate-200 bg-slate-50 px-4 py-3">
        {!isEdit && (
          <Button type="button" variant="primary" loading={isPending} onClick={handleSaveAndAdd}>
            + Save And Add
          </Button>
        )}
