...
from apps.common.models.models import Currency
...
class CustomUser(AbstractUser):
    ...
    is_verified = models.BooleanField(
        default=False,
        help_text=_(
            "Designates whether the user's account has been verified (e.g., email/phone)."
        ),
    )
    ...
    profile_content_type = models.ForeignKey(
        ...
        help_text=_(
            "The content type of the associated Individual or Company profile."
        ),
    )
    ...
    can_send_email = models.BooleanField(
        default=True,
        help_text=_(
            "Designates whether the user is allowed to send emails from the system."
        ),
    )
    ...
    roles = models.ManyToManyField(
        ...
        related_name="users",
        help_text=_(
            "The specific roles assigned to this user, granting specific permissions."
        ),
    )
    ...
    def clean(self):
        ...
        if not self.is_staff and not self.client:
            raise ValidationError(_("Non-staff users must be assigned to a client."))
...
class Role(models.Model):
    ...
    def get_all_permissions(self):
        ...
        return {
            f"{p.content_type.app_label}.{p.codename}" for p in self.permissions.all()
        }
...
class UserSetting(BaseModel):
    ...
    dark_mode_enabled = models.BooleanField(
        default=False, help_text=_("Enable dark mode for the user interface.")
    )
    email_notifications_enabled = models.BooleanField(
        default=True, help_text=_("Receive email notifications for system events.")
    )
