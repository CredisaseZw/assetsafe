...
class UserSettingInline(admin.StackedInline):
    ...
    fieldsets = (
        (None, {"fields": ("dark_mode_enabled", "email_notifications_enabled")}),
        (
            "Advanced",
            {
                "classes": ("collapse",),
...
class CustomUserAdmin(UserAdmin):
    ...
    search_fields = (
        ...
        "email",
        "client__name",
        "client__client_object__first_name",
    ...
    fieldsets = (
        ...
        (_("Client Association"), {"fields": ("client",), "classes": ("collapse",)}),
    )
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if "client" in form.base_fields and not request.user.is_superuser:
            if request.user.client:
                form.base_fields["client"].queryset = form.base_fields[
                    "client"
                ].queryset.filter(pk=request.user.client.pk)
            else:
                form.base_fields["client"].queryset = form.base_fields[
                    "client"
                ].queryset.none()
        ...
        if "profile_content_type" in form.base_fields:
            form.base_fields["profile_content_type"].queryset = (
    ...
    def send_welcome_email(self, request, queryset):
        ...
        self.message_user(
            request, f"Welcome emails will be sent to {queryset.count()} users."
        )
