...
class SuburbViewSerializer(serializers.ModelSerializer):
    ...
    country = serializers.CharField(source="city.province.country.name", read_only=True)
...
class CountrySerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.context.get("include_nested", False):
            self.fields["provinces"] = MinimalProvinceSerializer(
...
class AddressSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        raise NotImplementedError(
            "Address creation should be handled by the parent serializer"
        )
    ...
    def update(self, instance, validated_data):
        ...
        """
        raise NotImplementedError(
            "Address update should be handled by the parent serializer"
        )
...
class NoteSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not content_type or not object_id:
            raise serializers.ValidationError(
                "Content type and object ID are required for note creation."
            )
